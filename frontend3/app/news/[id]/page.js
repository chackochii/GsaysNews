      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-6 md:p-10 lg:px-32 bg-white">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6 text-gray-600 border-b border-gray-200 pb-4">
            {news.author && <span><strong>By</strong> <span className="text-blue-600 font-medium">{news.author}</span></span>}
            {news.date && <><span>•</span><span>{moment(news.date).format("DD/MM/yyyy")}</span></>}
          </div>
          <div className="prose prose-lg max-w-none text-gray-800 leading-relaxed">
            <p>{news.articleContent}</p>
            {news.extraImage && <img src={news.extraImage} alt="Extra visual" className="rounded-xl shadow-lg my-8"/>}
          </div>
